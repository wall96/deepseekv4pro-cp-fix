...
class CompressorBackendMixin:
    ...
    def forward_core_compressor(
        ...
        _cp_debug_dump(
            "H_core_compressor_out_loc_before_truncate",
            out_loc,
            layer_id=layer_id,
            extra={"ratio": compressor.ratio, "new_compressed_kv_shape": tuple(new_compressed_kv.shape)},
        )
        if out_loc.shape[0] > new_compressed_kv.shape[0]:
            out_loc = out_loc[: new_compressed_kv.shape[0]]
        _cp_debug_dump(
            "I_core_compressor_out_loc_after_truncate",
            out_loc,
            layer_id=layer_id,
            extra={"ratio": compressor.ratio},
        )
    ...
    def forward_indexer_compressor(
        ...
        _cp_debug_dump(
            "H_indexer_compressor_out_loc_before_truncate",
            out_loc,
            layer_id=layer_id,
            extra={"ratio": compressor.ratio, "new_compressed_kv_shape": tuple(new_compressed_kv.shape)},
        )
        if out_loc.shape[0] > new_compressed_kv.shape[0]:
            out_loc = out_loc[: new_compressed_kv.shape[0]]
        _cp_debug_dump(
            "I_indexer_compressor_out_loc_after_truncate",
            out_loc,
            layer_id=layer_id,
            extra={"ratio": compressor.ratio},
        )
