...
class DeepseekV4AttnBackend(
    ...
    def expand_prefill_casually(
        ...
        if padded_num_tokens is not None and padded_num_tokens > num_tokens:
            ...
            # Padding tokens must get positions strictly beyond the real
            # range. A constant fill (e.g. seq_lens_casual=1 -> position 0)
            # aliases with a real token's position, which corrupts CP
            # round-robin split results once padding slots land on a
            # different cp_rank than their aliased real token (see
            # dsv4pro/log-analysis.md section 14).
            pad_seq_lens = seq_lens_casual[-1:] + torch.arange(
                1, pad_size + 1, **self.cuda_int32_kwargs
            )
    ...
    def _expand_prefill_casually_vectorized(
        ...
        if padded_num_tokens is not None and padded_num_tokens > num_tokens:
            pad_size = padded_num_tokens - num_tokens
            # See expand_prefill_casually above: padding must not alias a
            # real token's position (dsv4pro/log-analysis.md section 14).
            pad_seq_lens = seq_lens_casual[-1:] + torch.arange(
                1, pad_size + 1, **self.cuda_int32_kwargs
            )
            seq_lens_casual = torch.cat([seq_lens_casual, pad_seq_lens])
