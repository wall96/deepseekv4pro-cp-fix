...
# --- Temporary CP/PP precision debugging instrumentation -------------------
# See python/sglang/srt/utils/cp_pp_debug.py for what this does and how it's
# gated (SGLANG_CP_DEBUG_DUMP=1). Throwaway diagnostic, not a shipped feature
# -- delete once the CP/PP precision-loss root cause is found and fixed.
from sglang.srt.utils.cp_pp_debug import cp_debug_dump as _cp_debug_dump

# --- end CP/PP precision debugging instrumentation --------------------------
    ...
    def _forward_prepare(
        ...
        if do_fused_store:
            ...
            if unified:
                ...
                _cp_debug_dump("E_post_kv_allgather_rerange", kv, layer_id=self.layer_id)
                attn_backend.store_cache(
                    layer_id=self.layer_id,
                    swa_k=kv,
                    forward_batch=forward_batch,
...
class DeepseekV4DecoderLayer(nn.Module):
    ...
    def forward(
        ...
        _cp_debug_dump("A_post_attn_pre_moe_gather", hidden_states, layer_id=self.layer_id)
        _use_tp_moe_gather = (
            not _use_cp
        ...
        _cp_debug_dump("C_post_mlp_pre_reduce_scatter", hidden_states, layer_id=self.layer_id)
        if _use_cp and get_moe_a2a_backend().is_none():
            hidden_states = dsa_cp_reduce_scatter_hidden_states(hidden_states)
            _cp_debug_dump("D_post_reduce_scatter", hidden_states, layer_id=self.layer_id)
        elif _use_tp_moe_gather:
        ...
        if self.dsa_enable_prefill_cp:
            if can_dsa_cp_split(len(input_ids), self.cp_size, True, forward_batch):
                ...
                if is_dsa_prefill_cp_round_robin_split():
                    ...
                    # Pre-reindex: the full (un-sliced) positions this stage's
                    # local scheduler sees for this microbatch, before we cut
                    # it down to just this cp_rank's round-robin subset. If
                    # PP stages disagree on what the "full" sequence even
                    # looks like for the same logical microbatch, it'll show
                    # up here first.
                    _cp_debug_dump(
                        "F_pre_cp_reindex_positions",
                        core_meta.positions_casual,
                        layer_id=-1,
                        extra={
                            "meta_id": id(core_meta),
                            "num_input_ids": len(input_ids),
                        },
                    )
                    core_meta.apply_cp_reindex()
                    # Post-reindex: this rank's round-robin slice. Compare the
                    # *set* of position values here across pp_rank (should be
                    # identical for the same cp_rank on every stage) and
                    # across consecutive calls on the same stage (should
                    # never see a slice belonging to a different microbatch
                    # bleed in if pipelining ever overlaps this metadata
                    # object across mb_ids).
                    _cp_debug_dump(
                        "G_post_cp_reindex_positions",
                        core_meta.positions_casual,
                        layer_id=-1,
                        extra={"meta_id": id(core_meta)},
                    )
