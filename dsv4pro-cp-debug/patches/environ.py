...
class Envs:
    ...
    # DSV4 prefill-CP investigation: keep SGLANG_OPT_FLASHMLA_SPARSE_PREFILL enabled even
    # when DeepSeekV4 context-parallel is on, instead of validate_deepseek_v4_cp's
    # unconditional .set(False). Lets prefill go through _forward_prefill_sparse
    # (flash_mla_sparse_fwd) under CP to test whether the flash_mla_with_kvcache path CP
    # is otherwise forced onto is the source of the round-robin-split precision bug.
    SGLANG_TEST_CP_KEEP_SPARSE_PREFILL = EnvBool(False)
