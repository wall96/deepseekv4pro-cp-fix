...
from sglang.srt.utils.cp_pp_debug2 import cp_debug_dump as _cp_debug_dump
...
class DeepseekV4DecoderLayer(nn.Module):
    ...
    def forward(
        ...
        _cp_debug_dump("T1_post_self_attn", hidden_states, layer_id=self.layer_id)
        ...
        if use_fused:
            ...
            if fused_mhc is not None:
                ...
                norm_fused = True
            _cp_debug_dump("T2_pre_cp_dispatch", hidden_states, layer_id=self.layer_id)
        else:
            hidden_states = self.hc_post(hidden_states, residual, post, comb)
            _cp_debug_dump("T2_post_hc_post_attn", hidden_states, layer_id=self.layer_id)
            residual = hidden_states
            hidden_states, post, comb, norm_fused = self.hc_pre(
                hidden_states,
                self.hc_ffn_fn,
        ...
        if _use_cp:
            if get_moe_a2a_backend().is_none():
                _cp_debug_dump("T3_pre_cp_gather", hidden_states, layer_id=self.layer_id)
                hidden_states = dsa_cp_gather_hidden_states(hidden_states)
                _cp_debug_dump("T3_post_cp_gather", hidden_states, layer_id=self.layer_id)
            else:
        ...
        _cp_debug_dump("T4_pre_mlp", hidden_states, layer_id=self.layer_id)
        ...
        _cp_debug_dump("T4_post_mlp", hidden_states, layer_id=self.layer_id)
        if _use_cp and get_moe_a2a_backend().is_none():
            _cp_debug_dump("T5_pre_cp_reduce_scatter", hidden_states, layer_id=self.layer_id)
            hidden_states = dsa_cp_reduce_scatter_hidden_states(hidden_states)
            _cp_debug_dump("T5_post_cp_reduce_scatter", hidden_states, layer_id=self.layer_id)
        elif _use_tp_moe_gather:
        ...
        if not use_fused:
            hidden_states = self.hc_post(hidden_states, residual, post, comb)
            _cp_debug_dump("T6_layer_output", hidden_states, layer_id=self.layer_id)
        ...
        _cp_debug_dump("T6_layer_output_deferred", hidden_states, layer_id=self.layer_id)
