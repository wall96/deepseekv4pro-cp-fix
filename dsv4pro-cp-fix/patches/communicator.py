...
class LayerCommunicator:
    ...
    def should_use_reduce_scatter(self, forward_batch: ForwardBatch):
        ...
        if mla_use_prefill_cp(forward_batch):
            return True
        # round-robin-split DSA prefill-CP runs plain TP-sharded MoE
        # (ep_size=1, no DeepEP dispatch), which must do its own full TP
        # all-reduce; only in-seq-split (DeepEP) is allowed to skip it here.
        # See sgl-project/sglang#20360 (fixed for the old NSA-named
        # communicator, never ported to the DSA rename).
        if dsa_use_prefill_cp(forward_batch) and not is_dsa_prefill_cp_round_robin_split():
            return True
