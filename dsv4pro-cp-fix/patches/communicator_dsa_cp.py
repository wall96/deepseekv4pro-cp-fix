...
def dsa_cp_reduce_scatter_hidden_states(hidden_states: torch.Tensor):
    ...
    if not is_dsa_prefill_cp_round_robin_split():
        # in-seq-split (DeepEP) has MoE skip its own TP all-reduce (see
        # LayerCommunicator.should_use_reduce_scatter), so this reduce-scatter
        # is what combines the TP-partial sums. round-robin-split's MoE now
        # does its own full all-reduce (see should_use_reduce_scatter), so
        # `input_hidden_states` is already complete and identical on every CP
        # rank here -- an extra reduce-scatter would just sum cp_size
        # redundant copies of the same value together. See sgl-project/sglang#20360.
        ...
        *,
        ...
        if hidden_states.shape[0] != 0:
            if context.attn_tp_size > 1:
                # o_proj is a RowParallelLinear: each attn-TP rank only holds
                # a partial sum until this all-reduce runs. Was silently
                # missing here (class docstring promises it, body never did
                # it) -- see sgl-project/sglang#20360.
